...
async function verifyUsers() {
  ...
  try {
    ...
    if (johnResult.rows.length > 0) {
      console.log(
        "  ✅ Found:",
        johnResult.rows[0].display_name,
        "(",
        johnResult.rows[0].email,
        ")"
      );
    } else {
    ...
    if (aliceResult.rows.length > 0) {
      console.log(
        "  ✅ Found:",
        aliceResult.rows[0].display_name,
        "(",
        aliceResult.rows[0].email,
        ")"
      );
    } else {
    ...
    const allUsers = await pool.query(
      "SELECT user_id, email, display_name FROM users ORDER BY email"
    );
    ...
    const convResult = await pool.query(
      "SELECT conversation_id, conversation_type FROM conversations"
    );
